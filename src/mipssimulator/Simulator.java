...
import mipssimulator.KernelLoader;
import mipssimulator.KernelLoaderException;
...
{	
	...
	public static void main(String argv[])
	{
		...
		for (int i = 0; i < argv.length; i++) {
			if (argv[i].charAt(0) == '-') {
				if (argv[i].charAt(1) == 't') traceMode = true;
                                ...
                                else if (argv[i].charAt(1) == 'd') debugMode = true;
			}
			else {
				bootFilename = argv[i];
			}
		...
		System.out.println(", trace="+traceMode+", verbose="+verboseMode+", debug="+debugMode);
		try {
