...
import java.lang.*;
import java.util.*;
import mipssimulator.TLBEntry;
import mipssimulator.Instruction;
import mipssimulator.CPRegUnavailableException;
...
public class CoProc0 extends Object {
    ...
    static public final String regNames[] = {
        "Index", "Random", "EntryLo", "c0_rsvd3",
        "Context", "c0_rsvd5", "c0_rsvd6", "c0_rsvd7",
        "BadVAddr", "c0_rsvd9", "EntryHi", "c0_rsvd11",
        "SR", "Cause", "EPC", "PRId",
        "c0_rsvd16", "c0_rsvd17", "c0_rsvd18", "c0_rsvd19",
        "c0_rsvd20", "c0_rsvd21", "c0_rsvd22", "c0_rsvd23",
        "c0_rsvd24", "c0_rsvd25", "c0_rsvd26", "c0_rsvd27",
        "c0_rsvd28", "c0_rsvd29", "c0_rsvd30", "c0_rsvd31"
    };
    ...
    public void interrupt(int exception, int no, int pc, boolean braDelay) {
        ...
        register[STATUS] |= (tmp << 2);             // KUcurr=0 (kmode), IEcurr=0 (int disabl.)
        // set epc
        register[EPC] = pc;
    ...
    public void handleTLBInstruction(Instruction i) {
        ...
        case Instruction.OP_TLBP:
            for (j = 0; j < NOFTLBENTRIES; j++) {
                if ((tlb[j].lo == register[TLBLO]) && (tlb[j].hi == register[TLBHI])){
                    register[INDEX] = j << 8;
                    return;
                }
            }
            register[INDEX] = register[INDEX] | 0x80000000; // set probe bit 31
            break;
        default:
        }
    }
    ...
    public int translate(int virt, boolean write) throws MemoryException {
        ...
        if (virt >= 0 && virt <= 0x7fffffff) { // kernel/user mapped
            phys = tlbLookup(virt, false, write);
        } else {
            // check for kernel mode first
            if ((register[STATUS] & KERNELMODE) != 0) {
                if (write) throw new AddressErrorStoreException("access violation at " + Integer.toHexString(virt));
                else throw new AddressErrorLoadException("access violation at " + Integer.toHexString(virt));
            }
            if (virt > 0xc0000000 && virt <= 0xffffffff) { // kernel mapped
                phys = tlbLookup(virt, true, write);
            }
    ...
    final int tlbLookup(int virt, boolean kSeg2, boolean writeAccess) throws MemoryException {
    ...
    final int physical(int virt, TLBEntry e, boolean writeAccess) throws MemoryException {
        ...
        return ((tlb[tlbHint].lo & PAGEFRAMEMASK) | (virt & PAGEOFFSETMASK));
    }

    final int randomIndex() {
        return ((random.nextInt() % (NOFTLBENTRIES - RESERVEDENTRIES)) + RESERVEDENTRIES);
    }
}
